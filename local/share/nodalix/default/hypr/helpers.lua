...
local function command_from(value, description)
  ...
  if value.nodalix then
    return "nodalix-launch-" .. value.nodalix
  ...
  elseif value.tui then
    if value.focus then
      return "nodalix-launch-or-focus-tui " .. shell_quote(value.tui)
    else
      return "nodalix-launch-tui " .. shell_quote(value.tui)
    end
  end
...
function o.launch_webapp(url)
  return "nodalix-launch-webapp " .. shell_quote(url)
end

function o.launch_webapp_sole(name, url)
  return "nodalix-launch-or-focus-webapp " .. shell_quote(name) .. " " .. shell_quote(url)
end

function o.launch_sole(match, command)
  return "nodalix-launch-or-focus " .. shell_quote(match) .. " " .. shell_quote(o.launch(command))
end

function o.bind_menu(keys, description, menu, options)
  o.bind(keys, description, menu and ("nodalix-menu " .. menu) or "nodalix-menu", options)
end

function o.bind_toggle(keys, description, toggle, options)
  o.bind(keys, description, "nodalix-toggle-" .. toggle, options)
end
