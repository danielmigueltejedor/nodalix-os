...
// with nmcli, while Tailscale is queried through its own local CLI because it is
// a separate daemon and never appears in `nmcli connection show`.
Item {
    id: root
    ...
    onActiveChanged: {
        if (!active) return
        if (_vpnAvailable) {
            _vpnScan.running = false
            _vpnScan.running = true
        }
        if (_tailscaleAvailable) {
    ...
    readonly property bool _tailscaleAvailable: DependencyService.available("tailscale")
    ...
    // Tailscale is not a NetworkManager VPN, so detect and control it directly.
    ...
    function _toggleTailscale() {
        if (tailscaleBusy || !_tailscaleAvailable) return
    ...
    Timer {
        interval: 10000
        repeat: true
        running: root.active && root._tailscaleAvailable
        onTriggered: {
            root._tailscaleScan.running = false
            root._tailscaleScan.running = true
        }
