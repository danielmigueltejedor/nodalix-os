...
Item {
    ...
    property string localSendTarget: ""
    ...
    FileDialog {
        id: _localSendFiles
        title: I18n.tr("Choose files to send")
        fileMode: FileDialog.OpenFiles
        onAccepted: {
            const paths = []
            for (let i = 0; i < selectedFiles.length; ++i) paths.push(root._localPath(selectedFiles[i]))
            LocalSendService.send(root.localSendTarget, paths)
        }
    }
    ...
    onWifiSelectedSsidChanged: {
        const editing = wifiSelectedSsid !== ""
        PopoutService.pinned = editing
        PopoutService.textActive = editing
        if (!editing) { wifiConnectState = "idle"; wifiConnectError = "" }
    }
    ...
    onQsKeyChanged: {
        ...
        if (qsKey === "wifi") { _wifiState.running = true; _wifiScan.running = true }
            ...
            _tailscaleScan.running = false
            _tailscaleScan.running = true
        }
    }
    ...
    property Process _wifiScan: Process {
        command: ["sh", "-c", "nmcli -t -f IN-USE,SIGNAL,SSID device wifi list 2>/dev/null; printf '\\n--KNOWN--\\n'; nmcli -t -f NAME,TYPE connection show 2>/dev/null"]
        running: false
        stdout: StdioCollector {
            onStreamFinished: {
                const sections = text.split("\n--KNOWN--\n")
                const known = {}
                for (const ln of (sections[1] || "").trim().split("\n")) {
                    const cut = ln.lastIndexOf(":")
                    if (cut > 0 && ln.slice(cut + 1).indexOf("wireless") >= 0)
                        known[ln.slice(0, cut)] = true
                }
                // Merge by SSID across BSSIDs: keep max signal, OR the in-use flag.
                const byId = {}
                for (const ln of (sections[0] || "").trim().split("\n")) {
                    if (!ln) continue
                    const f = ln.split(":")
                    ...
                    if (byId[ssid]) {
                        ...
                        byId[ssid] = { ssid: ssid, signal: sig, active: act, known: known[ssid] === true }
                    }
                }
    ...
    property string _wifiConnectOutput: ""
    function _connectWifi(ssid, password, known) {
        if (!ssid || wifiConnectState === "connecting") return
        wifiConnectState = "connecting"
        wifiConnectError = ""
        _wifiConnectOutput = ""
        _wifiConnect.command = known
            ? ["nmcli", "connection", "up", "id", ssid]
            : ["nmcli", "device", "wifi", "connect", ssid, "password", password]
        _wifiConnect.running = false
        _wifiConnect.running = true
    }
    property Process _wifiConnect: Process {
        running: false
        stdout: StdioCollector { onStreamFinished: root._wifiConnectOutput += text }
        stderr: StdioCollector { onStreamFinished: root._wifiConnectOutput += text }
        onExited: (code, status) => {
            if (code === 0) {
                root.wifiConnectState = "success"
                root.wifiSelectedSsid = ""
            } else {
                root.wifiConnectState = "error"
                root.wifiConnectError = root._wifiConnectOutput.trim() || I18n.tr("Incorrect password or connection failed")
                if (!root.wifiSelectedKnown) {
                    root._forgetFailed.command = ["nmcli", "connection", "delete", "id", root.wifiSelectedSsid]
                    root._forgetFailed.running = false
                    root._forgetFailed.running = true
                }
                PopoutService.pinned = true
                PopoutService.textActive = true
            }
            root._rescanKey = "wifi"
            root._rescanTimer.restart()
        }
    }
    property Process _forgetFailed: Process { running: false }
    // VPN connection list
    ...
    property Process _tailscaleScan: Process {
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                try {
                    ...
                    root.tailscaleDetected = true
                    root.tailscaleActive = data.BackendState === "Running"
                    root.tailscaleIp = (data.TailscaleIPs && data.TailscaleIPs.length > 0)
                        ? data.TailscaleIPs[0] : ""
                } catch (e) {
                    root.tailscaleDetected = false
                    root.tailscaleActive = false
                    root.tailscaleIp = ""
                }
            }
        }
    }
    property Process _tailscaleAction: Process {
        running: false
        onRunningChanged: root.tailscaleBusy = running
        onExited: {
            root._tailscaleScan.running = false
            root._tailscaleScan.running = true
        }
    }
    function _toggleTailscale() {
        if (tailscaleBusy) return
        _tailscaleAction.command = ["tailscale", tailscaleActive ? "down" : "up"]
        _tailscaleAction.running = true
    }

    // External Bluetooth app. Network management stays inside Nodalix.
    ...
    function _formatUptime(totalSeconds) {
        ...
        if (days > 0) parts.push(days + " " + I18n.tr(days === 1 ? "day" : "days"))
        if (hours > 0) parts.push(hours + " " + I18n.tr(hours === 1 ? "hour" : "hours"))
        if ((days === 0 && minutes > 0) || parts.length === 0)
            parts.push(minutes + " " + I18n.tr(minutes === 1 ? "minute" : "minutes"))

        return parts.slice(0, 2).join(", ")
    }
    property Process _sysInfo: Process {
        command: ["sh", "-c", "whoami; printf '%s\\n' \"$HOME\"; awk '{print int($1)}' /proc/uptime"]
        running: false
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                root.userName = l[0] ?? ""
                root.homeDir  = l[1] ?? ""
                root.uptimeSeconds = Number.parseInt(l[2] ?? "0", 10) || 0
            }
        }
    }
    ...
    RowLayout {
        ...
        Item {
            ...
            Layout.fillHeight: true
            // The rail is the tallest column. Making it participate in layout
            // keeps the clock, calendar and player aligned with its bottom.
            Layout.minimumHeight: _qsRail.height
        }
    ...
    Rectangle {
        ...
        visible: root.tab === "home"
        width: 48
        height: _qsIcons.implicitHeight + 16
        anchors {
            top:    _grid.top
            right:  parent.right
    ...
    Rectangle {
        ...
        // Account for the flyout's actual top position. Using the whole dashboard
        // height here let tall Wi-Fi content extend below the clipped root.
        readonly property real _maxH: Math.max(120, root.height - y - ThemeManager.spacingLg)
        ...
        Flickable {
            ...
            Connections {
                target: root
                function onWifiSelectedSsidChanged() {
                    if (root.wifiSelectedSsid !== "")
                        Qt.callLater(() => _flyoutFlick.contentY =
                            Math.max(0, _flyoutFlick.contentHeight - _flyoutFlick.height))
                }
            }
    ...
    // ── LocalSend / Nodalix nearby sharing ──────────────────────────────────
    Component {
        id: _localSendCmp
        Item {
            implicitWidth: 300
            implicitHeight: _localSendCol.implicitHeight
            ColumnLayout {
                id: _localSendCol
                ...
                MenuHeader {
                    title: "LocalSend"
                    on: LocalSendService.enabled
                    onToggled: LocalSendService.setEnabled(!LocalSendService.enabled)
                }

                Text {
                    visible: !LocalSendService.enabled
                    Layout.fillWidth: true
                ...
                Repeater {
                    model: LocalSendService.enabled ? LocalSendService.pending : []
                    delegate: Rectangle {
                        required property var modelData
                        Layout.fillWidth: true
                        implicitHeight: _incomingCol.implicitHeight + 16
                        radius: ThemeManager.chipRadius + 2
                        color: Qt.rgba(ThemeManager.primary.r, ThemeManager.primary.g, ThemeManager.primary.b, 0.12)
                        border.width: 1
                        border.color: Qt.rgba(ThemeManager.primary.r, ThemeManager.primary.g, ThemeManager.primary.b, 0.35)
                        ColumnLayout {
                            id: _incomingCol
                            ...
                            RowLayout {
                                Layout.fillWidth: true
                                spacing: 6
                                MenuFooter { Layout.fillWidth: true; Layout.topMargin: 0; text: I18n.tr("Decline"); onClicked: LocalSendService.decline(modelData.id) }
                                MenuFooter { Layout.fillWidth: true; Layout.topMargin: 0; text: I18n.tr("Accept"); onClicked: LocalSendService.accept(modelData.id) }
                            }
                        }
                    }
                }
                ...
                Text {
                    visible: LocalSendService.enabled && LocalSendService.devices.length === 0
                    Layout.fillWidth: true
                ...
                Repeater {
                    model: LocalSendService.enabled ? LocalSendService.devices : []
                    delegate: RowLayout {
                        required property var modelData
                        Layout.fillWidth: true
                        spacing: 4
                ...
                Text {
                    visible: LocalSendService.transfer.state !== "idle" && LocalSendService.transfer.message !== ""
                    Layout.fillWidth: true
                ...
                MenuFooter {
                    visible: LocalSendService.enabled
                    text: I18n.tr("Search for devices")
                    onClicked: { LocalSendService.announce(); LocalSendService.refresh() }
                }
                MenuFooter {
                    Layout.fillWidth: true
                    Layout.topMargin: 0
                    text: I18n.tr("Open received folder")
                    onClicked: LocalSendService.openReceivedFolder()
                }
                Text {
                    visible: LocalSendService.enabled
                    Layout.fillWidth: true
    ...
    Component {
        ...
        Item {
            implicitWidth: 300
            implicitHeight: _captureCol.implicitHeight
            ColumnLayout {
                id: _captureCol
                anchors { left: parent.left; right: parent.right; top: parent.top }
                spacing: 7
                ...
                Rectangle {
                    ...
                    implicitHeight: 52
                    radius: ThemeManager.chipRadius
                    border.width: ScreenRecorderService.recording ? 1 : 0
                    border.color: Qt.rgba(ThemeManager.error.r, ThemeManager.error.g, ThemeManager.error.b, 0.65)
                    color: ScreenRecorderService.recording
                        ? Qt.rgba(ThemeManager.error.r, ThemeManager.error.g, ThemeManager.error.b, _recordMa.containsMouse ? 0.18 : 0.11)
                        : (_recordMa.containsMouse
                    ...
                    RowLayout {
                        ...
                        ColumnLayout {
                            ...
                            spacing: 0
                            Text {
                                text: ScreenRecorderService.recording
                                    ? (ScreenRecorderService.stopping ? I18n.tr("Finishing recording…")
                        ...
                        Rectangle {
                            ...
                            SequentialAnimation on opacity {
                                running: ScreenRecorderService.recording && !ScreenRecorderService.paused && !ScreenRecorderService.stopping
                                loops: Animation.Infinite
                    ...
                    MouseArea {
                        id: _recordMa
                        anchors.fill: parent
                        enabled: !ScreenRecorderService.recording && !ScreenRecorderService.starting
                        hoverEnabled: true
                        cursorShape: enabled ? Qt.PointingHandCursor : Qt.ArrowCursor
                        onClicked: {
                ...
                MenuFooter {
                    visible: ScreenRecorderService.starting
                    Layout.fillWidth: true; Layout.topMargin: 0
                    text: I18n.tr("Cancel")
                    onClicked: ScreenRecorderService.cancelSelection()
                }
                RowLayout {
                    visible: ScreenRecorderService.recording
                    Layout.fillWidth: true
                    spacing: 6
                    MenuFooter {
                        visible: !ScreenRecorderService.stopping
                        Layout.fillWidth: true; Layout.topMargin: 0
                        text: ScreenRecorderService.paused ? I18n.tr("Resume") : I18n.tr("Pause")
                        onClicked: ScreenRecorderService.togglePause()
                    }
                    MenuFooter {
                        Layout.fillWidth: true; Layout.topMargin: 0
                        text: ScreenRecorderService.stopping ? I18n.tr("Finishing recording…") : I18n.tr("Finish recording")
                        enabled: !ScreenRecorderService.stopping
    ...
    Rectangle {
        ...
        ColumnLayout {
            ...
            RowLayout {
                ...
                Layout.topMargin: 4
                spacing: 6
                ConfirmBtn { label: I18n.tr("Cancel");  onClicked: root._confirmAction = "" }
                ConfirmBtn { label: I18n.tr("Confirm"); danger: true; onClicked: root._runConfirm() }
            }
        }
    }
    ...
    Rectangle {
        ...
        readonly property var _reminders: {
            const _ = CalendarService.reminders
            return root.calSelectedDate !== "" ? CalendarService.reminders : []
        }
        ...
        Flickable {
            ...
            ColumnLayout {
                ...
                ColumnLayout {
                    ...
                    CalField { id: _evTitle; placeholder: I18n.tr("Title") }
                    RowLayout {
                        Layout.fillWidth: true
                        spacing: 6
                    ...
                    CalField { id: _evLoc; placeholder: I18n.tr("Location (optional)") }
                ...
                Repeater {
                    model: root.calCreating ? [] : _calPanel._reminders
                    delegate: Rectangle {
                        required property var modelData
                        Layout.fillWidth: true
                        implicitHeight: 34
    ...
    Component {
        ...
        Item {
            ...
            MenuFooter { text: I18n.tr("Open blueman-manager"); onClicked: root._launch(root._blueman) }
            }
        }
    }
        ...
        Item {
            ...
            Repeater {
                ...
                delegate: MenuRow {
                    ...
                    onClicked: {
                        if (modelData.active) {
                            root._nm(["nmcli", "connection", "down", "id", modelData.ssid], "wifi")
                            root.wifiSelectedSsid = ""
                        } else if (modelData.known) {
                            root.wifiSelectedKnown = true
                            root.wifiSelectedSsid = modelData.ssid
                            root._connectWifi(modelData.ssid, "", true)
                        } else {
                            root.wifiSelectedKnown = false
                            root.wifiConnectState = "idle"
                            root.wifiConnectError = ""
                            root.wifiSelectedSsid = modelData.ssid
                            _wifiPassword.text = ""
                            _wifiPassword.forceActiveFocus()
                        }
                    }
                }
            }
            ...
            Rectangle {
                ...
                Layout.fillWidth: true
                Layout.topMargin: 6
                implicitHeight: _wifiPasswordCol.implicitHeight + 16
                radius: ThemeManager.chipRadius + 2
                color: ThemeManager.surfaceContainer
                border.width: 1
                border.color: Qt.rgba(ThemeManager.primary.r, ThemeManager.primary.g, ThemeManager.primary.b, 0.35)
                ...
                ColumnLayout {
                    ...
                    Rectangle {
                        ...
                        radius: ThemeManager.chipRadius
                        color: ThemeManager.surfaceContainerHigh
                        border.width: _wifiPassword.activeFocus ? 1 : 0
                        border.color: ThemeManager.primary
                        TextInput {
                            id: _wifiPassword
                            ...
                            clip: true
                            onActiveFocusChanged: PopoutService.textActive = activeFocus
                            enabled: root.wifiConnectState !== "connecting"
                            onAccepted: if (text.length > 0)
                                root._connectWifi(root.wifiSelectedSsid, text, false)
                            Text {
                                anchors.verticalCenter: parent.verticalCenter
                                visible: _wifiPassword.text === ""
                                text: I18n.tr("Password")
                                color: ThemeManager.onSurfaceVariant; opacity: 0.55
                    ...
                    Text {
                        visible: root.wifiConnectState === "error"
                        Layout.fillWidth: true
                    ...
                    RowLayout {
                        ...
                        MenuFooter {
                            text: I18n.tr("Cancel")
                            Layout.fillWidth: true; Layout.topMargin: 0
                            onClicked: { root.wifiSelectedSsid = ""; PopoutService.textActive = false }
                        }
                        MenuFooter {
                            text: root.wifiConnectState === "connecting" ? I18n.tr("Connecting…") : I18n.tr("Connect")
                            Layout.fillWidth: true; Layout.topMargin: 0
                            enabled: root.wifiConnectState !== "connecting"
                            onClicked: if (_wifiPassword.text.length > 0)
                                root._connectWifi(root.wifiSelectedSsid, _wifiPassword.text, false)
                        }
                    }
                }
            }
            ...
            MenuFooter {
                text: I18n.tr("Refresh networks")
                onClicked: root._wifiScan.running = true
            }
            }
        }
    }
        ...
        Item {
            ...
            MenuRow {
                ...
                trailing: root.tailscaleBusy
                    ? I18n.tr("Working…")
                    : (root.tailscaleActive
                        ? (I18n.tr("On") + (root.tailscaleIp !== "" ? " · " + root.tailscaleIp : ""))
                        : I18n.tr("Off"))
                onClicked: root._toggleTailscale()
            }
            ...
            MenuFooter {
                text: I18n.tr("Refresh connections")
                onClicked: {
                    root._vpnScan.running = true
                    root._tailscaleScan.running = false
                    root._tailscaleScan.running = true
                }
            }
    ...
    component QsIcon: Item {
        ...
        Image {
            ...
            layer.effect: MultiEffect {
                colorization: 1.0
                colorizationColor: qi.active ? ThemeManager.primary : ThemeManager.onSurfaceVariant
            }
        }
        MouseArea {
    ...
    component MetricsCard: Rectangle {
        ...
        ColumnLayout {
            ...
            Ring { Layout.fillWidth: true; label: I18n.tr("CPU");  frac: SystemMetricsService.cpu / 100; value: Math.round(SystemMetricsService.cpu) + "%" }
            Ring { Layout.fillWidth: true; label: I18n.tr("RAM");  frac: SystemMetricsService.ram / 100; value: Math.round(SystemMetricsService.ram) + "%" }
            Ring {
                Layout.fillWidth: true; label: I18n.tr("TEMP")
