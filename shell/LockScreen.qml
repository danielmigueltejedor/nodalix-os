...
WlSessionLock {
    ...
    readonly property var _fr: Qt.locale(I18n.localeName)
        ...
        Item {
            ...
            // Keep the generated thumbnail underneath the video so the lock
            // screen never flashes black while the decoder prepares its first
            // frame, and so unsupported media still has a useful fallback.
            Image {
                anchors.fill: parent
                source: LockService.wallpaperPreview
            ...
            Video {
                id: _wallVideo
                anchors.fill: parent
                visible: LockService.wallpaperAnimated
                opacity: visible && playbackState === MediaPlayer.PlayingState ? 1 : 0
                source: visible ? ("file://" + encodeURI(LockService.wallpaper)) : ""
