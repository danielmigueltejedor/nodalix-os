...
PanelWindow {
    ...
    readonly property var _incoming: LocalSendService.pending.length > 0 ? LocalSendService.pending[0] : null
    readonly property bool _incomingVisible: _incoming !== null &&
        (!Hyprland.focusedMonitor || Hyprland.focusedMonitor.name === root.modelData?.name)
    readonly property int _incomingW: 390
    readonly property int _incomingH: 164
    readonly property real _incomingX: Math.round((root.width - _incomingW) / 2)
    readonly property real _incomingY: Math.round(Math.max(ThemeManager.barHeight + 24, (root.height - _incomingH) / 2))
    ...
    // `open` identifies keyboard mode. `wpOpen` can also be set by a mouse
    // click; treating every picker as keyboard-open makes the full-screen
    // dismiss area appear during that same click and immediately close it.
    readonly property bool _toolsKbdActive: ToolsService.open
    ...
    mask: Region {
        ...
        Region {
            ...
            width: root._incomingVisible ? root._incomingW : 0
            height: root._incomingVisible ? root._incomingH : 0
        }
    }
    ...
    Rectangle {
        ...
        layer.enabled: true
        layer.effect: Elevation { level: 5 }
        ...
        ColumnLayout {
            ...
            RowLayout {
                ...
                Repeater {
                    model: [
                        { label: I18n.tr("Decline"), accept: false },
                        { label: I18n.tr("Accept"), accept: true }
                    ]
                    delegate: Rectangle {
                        required property var modelData
