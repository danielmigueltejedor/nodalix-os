...
QtObject {
    ...
    readonly property bool wallpaperAnimated:
        WallpaperService.current !== "" && WallpaperService.currentAnimated
    readonly property string wallpaperPreview:
        wallpaperAnimated ? WallpaperService.thumbnailFor(wallpaper) : wallpaper
    ...
    property PamContext _pam: PamContext {
        ...
        onCompleted: (result) => {
            ...
            if (result === PamResult.Success) {
                ...
                root.error = I18n.tr("Too many attempts")
            } else {
                root.error = I18n.tr("Incorrect password")
            }
        }
        onError: (msg) => { root.busy = false; root._pw = ""; root.error = I18n.tr("Authentication error") }
    }
