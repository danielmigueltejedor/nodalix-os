...
// CPU %, RAM %, and temperature via /proc + lm_sensors/sysfs.
...
QtObject {
    ...
    property Process _proc: Process {
        command: ["sh", "-c",
            ...
            "temp=$(sensors -u 'k10temp-*' 2>/dev/null | awk '/temp1_input:/ { printf \"%.0f\\n\", $2 * 1000; exit }'); " +
            "[ -n \"$temp\" ] || temp=$(sensors -u 'coretemp-*' 2>/dev/null | awk '/temp1_input:/ { printf \"%.0f\\n\", $2 * 1000; exit }'); " +
