...
pub fn list_monitors() -> Result<Vec<Monitor>, String> {
    ...
    let value: Value = serde_json::from_str(&output)
        .map_err(|e| format!("No se pudo leer monitores: {e}"))?;
...
fn hypr_monitor_rule(spec: &str) -> Result<(), String> {
    ...
    run_command_status("hyprctl", &["keyword", "monitor", spec]).map_err(|first| {
        format!("{first}. Prueba: hyprctl exec \"keyword monitor {spec}\"")
    })
}
