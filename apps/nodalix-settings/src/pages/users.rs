...
pub fn build_users_page() -> gtk::Widget {
    ...
    let page = pages::page(
        "Usuarios",
        "Cuenta local, imagen de perfil y listado de usuarios del sistema.",
    );
    let status = StatusStrip::new();
    page.append(&status.root);

    let profile = pages::card("Usuario actual");
    profile.append(&avatar_section(&current, &status));
    ...
    let paths = pages::card("Rutas de avatar");
    for path in users::avatar_candidates(&current) {
        let state = if path.is_file() { "detectado" } else { "no existe" };
        paths.append(&pages::row(&path.display().to_string(), state));
    }
    page.append(&paths);
    ...
    if all.is_empty() {
        list.append(&pages::row("Estado", "No se pudieron leer usuarios locales"));
    } else {
        for user in all {
...
fn avatar_section(user: &LocalUser, status: &StatusStrip) -> gtk::Box {
    ...
    let pick = gtk::Button::with_label("Elegir imagen…");
