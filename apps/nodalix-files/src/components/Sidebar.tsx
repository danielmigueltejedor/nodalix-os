...
interface SidebarProps {
  ...
  onPreloadItem?: (item: SidebarItem) => void;
      ...
      {!collapsed && (
        <span className="min-w-0 flex-1">
          ...
          <span className="block truncate text-[10px] text-nodalix-muted/70">
            {sending ? "Enviando…" : device.device_model || device.ip}
          </span>
        </span>
      )}
    </button>
...
function SidebarButton({
  ...
  return (
    <button
      ...
      onMouseEnter={() => onPreloadItem?.(item)}
      onFocus={() => onPreloadItem?.(item)}
      onContextMenu={(e) => {
        e.preventDefault();
        e.stopPropagation();
            ...
            >
              ...
              {!collapsed && (
                <button
                  ...
                  title="Actualizar dispositivos LocalSend"
                  aria-label="Actualizar dispositivos LocalSend"
                  onClick={(event) => {
                    event.stopPropagation();
                    onRefreshLocalSend?.();
                  }}
