...
export default function Toolbar({
  ...
  const tabsRow = (
    <div className="nodalix-tabs-shell flex min-w-0 flex-1 items-center gap-1">
      <div
        className="nodalix-tabs-row flex min-w-0 flex-1 items-center gap-1 overflow-x-auto"
        onDragOver={(event) => {
          const tabId =
            draggingTabId ||
        ...
        onDrop={(event) => {
          ...
          if (lastTab && lastTab.id !== tabId) onTabReorder(tabId, lastTab.id);
    ...
    >
      <div className="nodalix-toolbar-row relative flex min-h-8 items-center gap-2">
        <div className="flex shrink-0 items-center gap-1">
          <NavBtn label="Atrás" disabled={!canGoBack} onClick={onBack}>
            <Chevron dir="left" />
          </NavBtn>
          <NavBtn label="Adelante" disabled={!canGoForward} onClick={onForward}>
            <Chevron dir="right" />
          </NavBtn>
        </div>
        ...
        >
          <button
            ...
            type="button"
            aria-label="Ajustes de Nodalix Files"
            aria-expanded={settingsOpen}
            title="Ajustes de Nodalix Files"
          ...
          {settingsOpen && (
            ...
            >
              <section>
                <p className="nodalix-settings-title">Apariencia</p>
