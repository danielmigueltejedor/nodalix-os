  ...
  return (
    ...
    >
      <div className="flex items-center gap-1">
        {c.show_minimize && (
          <ChromeBtn label="Minimizar" onClick={() => void win.minimize()}>
            <span className="mb-2 block h-0.5 w-3 bg-current" />
          </ChromeBtn>
        )}
        {c.show_maximize && (
          <ChromeBtn label="Maximizar" onClick={() => void win.toggleMaximize()}>
            <span className="block h-2.5 w-3 border border-current" />
          </ChromeBtn>
        )}
        {c.show_close && (
          <ChromeBtn label="Cerrar" danger onClick={() => void win.close()}>
