...
fn config_path() -> Result<PathBuf, String> {
    let home = std::env::var("HOME").map_err(|_| "No se ha definido HOME".to_string())?;
...
fn builtin_items(dirs: &SpecialDirs, config: &SidebarConfig) -> Vec<SidebarItem> {
    ...
    let builtins: Vec<(&str, &str, Option<String>)> = vec![
        ("home", "Inicio", Some(dirs.home.clone())),
        ("downloads", "Descargas", dirs.downloads.clone()),
        ("documents", "Documentos", dirs.documents.clone()),
        ("pictures", "Imágenes", dirs.pictures.clone()),
        ("videos", "Vídeos", dirs.videos.clone()),
        ("music", "Música", dirs.music.clone()),
        ("disks", "Discos", Some("nodalix://disks".into())),
        ("network", "Red", Some("nodalix://network".into())),
        ("trash", "Papelera", dirs.trash.clone()),
        ("data", "Datos", dirs.data.clone()),
    ];
...
pub fn pin_sidebar_path(path: String, label: Option<String>) -> Result<(), String> {
    ...
    let name = label.unwrap_or_else(|| path.rsplit('/').next().unwrap_or("Carpeta").to_string());
