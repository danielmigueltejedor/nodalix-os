...
fn collect_upload_files(
    ...
    let metadata = fs::symlink_metadata(path)
        .map_err(|e| format!("No se puede leer {}: {e}", path.display()))?;
    if metadata.is_dir() {
        let base = root_name
        ...
        for entry in fs::read_dir(path)
            .map_err(|e| format!("No se puede leer la carpeta {}: {e}", path.display()))?
        {
    ...
    let name = root_name
        ...
        .ok_or_else(|| "Ruta de archivo no válida".to_string())?;
...
fn post_json(url: &str, body: &Value) -> Result<Value, String> {
    ...
    let mut child = Command::new("curl")
        ...
        .stderr(Stdio::piped())
        .spawn()
        .map_err(|e| format!("No se pudo iniciar curl: {e}"))?;
    ...
    serde_json::from_slice(&output.stdout)
        .map_err(|e| format!("Respuesta de LocalSend no válida: {e}"))
}
...
fn send_to_localsend_device_blocking(
    ...
    if paths.is_empty() {
        return Err("No hay archivos seleccionados".into());
    }
    if !command_exists("curl") {
        return Err("Se necesita curl para enviar archivos con LocalSend".into());
    }
    ...
    if files.is_empty() {
        return Err("No hay archivos normales que enviar".into());
    }
    ...
    let session_id = response
        ...
        .ok_or_else(|| "LocalSend no devolvió un identificador de sesión".to_string())?;
    let tokens = response
        .get("files")
        .and_then(Value::as_object)
        .ok_or_else(|| "LocalSend no aceptó ningún archivo".to_string())?;

    for file in &files {
        let token = tokens
            .get(&file.id)
            .and_then(Value::as_str)
            .ok_or_else(|| format!("LocalSend no aceptó {}", file.name))?;
...
pub fn send_with_localsend(paths: Vec<String>) -> Result<(), String> {
    ...
    if !info.available {
        return Err("LocalSend no está instalado".into());
    }
