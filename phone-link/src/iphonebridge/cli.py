...
app = typer.Typer(
    ...
    help="Enlace Bluetooth entre iPhone y Nodalix.",
)
...
def doctor(verbose: bool = typer.Option(False, "-v", "--verbose")):
    ...
    ok = True
    try:
        bluez_setup.resolve_adapter()
    except Exception as e:
        log.error("No usable Bluetooth adapter: %s", e)
        ok = False
    ...
    # BlueZ installs obexd in different locations across distributions.
    obexd = shutil.which("obexd") or next((str(p) for p in (
        Path("/usr/lib/bluetooth/obexd"),
        Path("/usr/libexec/bluetooth/obexd"),
    ) if p.is_file()), None)
    if obexd is None:
        log.error("BlueZ OBEX is not installed")
        log.error("    → Arch: sudo pacman -S bluez-obex")
        ok = False
    else:
        log.info("BlueZ OBEX installed: %s", obexd)
